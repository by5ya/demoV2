...
class EditCreateProductPage(QDialog):
    def __init__(self, controller: MainWin, product=None):
        super().__init__()
        ...
        if product:
            ...
            self.del_pushButton_3.setVisible(False)
            self.id_lineEdit_6.setVisible(False)
            self.id_label_12.setVisible(False)

        categories = self.db.get_distinct("category", "products")
        self.cat_comboBox.addItems(categories)

        producers = self.db.get_distinct("producer", "products")
        self.prod_comboBox_2.addItems(producers)

        self.image = "picture.png"

        self.load_pushButton.clicked.connect(self.load_pic)
        self.back_pushButton_2.clicked.connect(lambda: self.accept())
        self.disc_spinBox_2.setMaximum(99)
        self.quant_spinBox.setMaximum(10000000)
        self.price_doubleSpinBox.setMaximum(10000000)

        if self.edit:
            self.name_lineEdit.setText(self.product['name'])
            self.unit_lineEdit_2.setText(self.product['unit'])
            self.desc_plainTextEdit.setPlainText(self.product['description'])
            self.price_doubleSpinBox.setValue(self.product['price'])
            self.quant_spinBox.setValue(self.product['quantity'])
            self.disc_spinBox_2.setValue(self.product['discount'])
            self.prod_comboBox_2.setCurrentText(self.product['producer'])
            self.cat_comboBox.setCurrentText(self.product['category'])
            self.seller_lineEdit_3.setText(self.product['seller'])
            self.id_lineEdit_6.setText(str(self.product['id']))
            self.id_lineEdit_6.setEnabled(False)
            self.articul_lineEdit_4.setText(self.product['articul'])
            self.articul_lineEdit_4.setEnabled(False)

            self.image = self.product['photo']

        self.pic_label_11.setPixmap(QPixmap(str(self.controller.base_path/self.image)))

        self.save_pushButton.clicked.connect(self.save)
        self.del_pushButton_3.clicked.connect(self.delete_product)

    def delete_product(self):
        if self.db.check_product(self.product['id']):
            res = warn_ok("Вы действительно хотите удалить товар?")
            if res:
                self.db.delete_product(self.product['id'])
                self.accept()
        else:
            warn_ok("Товар находиться в заказе! Удаление запрещено")


    def check(self):
        return all([self.name_lineEdit.text(), self.articul_lineEdit_4.text(), self.unit_lineEdit_2.text(), self.price_doubleSpinBox.value(), self.seller_lineEdit_3.text(), self.desc_plainTextEdit.toPlainText()])

    def save(self):
        if self.check():
            product = {
                "name": self.name_lineEdit.text(),
                "articul": self.articul_lineEdit_4.text(),
                "unit": self.unit_lineEdit_2.text(),
                "price" : self.price_doubleSpinBox.value(),
                "seller": self.seller_lineEdit_3.text(),
                "producer" : self.prod_comboBox_2.currentText(),
                "category" : self.cat_comboBox.currentText(),
                "discount" : self.disc_spinBox_2.value(),
                "quantity" : self.quant_spinBox.value(),
                "description" : self.desc_plainTextEdit.toPlainText(),
                "photo": self.image
            }
            if self.edit:
                try:
                    self.db.update_product(product, self.product['id'])
                    info_ok("Товар успешно обновлен")
                    self.accept()
    ...
    def load_pic(self):
        file_loader = QFileDialog(self)
        path, _ = file_loader.getOpenFileName(
            self,
            "Выберите изображение",
            str(Path(__file__).home()),
            "Images *.jpg *.jpeg *.png"
        )

        filename = Path(path).name
        dest_path = self.controller.base_path/filename

        if path != str(dest_path):
            shutil.copy2(path, dest_path)

        self.pic_label_11.setPixmap(QPixmap(str(dest_path)))
